# Setup Guides

This directory contains setup and configuration guides for various services and tools used in the documentation project.

## Contents

### Search Configuration
- **MEILISEARCH_SETUP.md** - Meilisearch initial setup guide
- **MEILISEARCH_CONFIG.md** - Meilisearch configuration reference
- **MEILISEARCH_INDEXING.md** - Meilisearch indexing guide
- **MEILISEARCH_SECURITY.md** - Meilisearch security configuration
- **MEILISEARCH_CORS_FIX.md** - CORS configuration troubleshooting

### Error Tracking
- **SENTRY_SETUP.md** - Sentry error tracking setup
- **SENTRY_SETUP_COMPLETE.md** - Sentry setup completion guide

### Domain & Deployment
- **CUSTOM_DOMAIN_SETUP.md** - Custom domain configuration
- **CUSTOM_DOMAIN_TROUBLESHOOTING.md** - Domain setup troubleshooting


---

**Note:** These are setup/configuration guides for developers and maintainers. They are not part of the public documentation site.

